library(ggplot2)
a<-read.table("/Users/wuzhongzi/Desktop/new_cc.txt",header=F)
a$V5=factor(a$V4,levels=c("ECP","EDP","EWB","CEC","KND","NAD","NAW","CTP","SAD","SAW","OUT"))
#小提琴&箱线图
p3=ggplot(a,aes(x=V5,y=V3,fill=V5)) +
  geom_violin(trim=T,color="white",width=1.0) + #绘制小提琴图, “color=”设置小提琴图的轮廓线的颜色(#不要轮廓可以设为white以下设为背景为白色，其实表示不要轮廓线)
  ...
  scale_fill_manual(values= c("#FFFF00", "#FFD700","#FF69B4","#00FFFF", "#00FF00","#008000","#8B0000", "#8A2BE2","#0000FF","#FF0000", "#666666"))+ #设置填充的颜色
  labs(title="", x="Subpopulation", y = "Novel SVs per sample",size=20) +
  theme_bw()+#把背景设置为白底
  theme(plot.title = element_text(hjust =0.5,colour="black",face="bold",size=18), # 将图表标题居中
       ...
       # axis.text.x=element_text(angle=0,hjust=1,colour="black",face="bold",size=14), #设置x轴刻度标签的字体显示倾斜角度为45度，并向下调整1(hjust = 1)，字体大小为14
        axis.text.y=element_text(hjust=0.5,colour="black",face="bold",size=12), #设置y轴刻度标签的字体簇，字体大小，字体样式为plain
